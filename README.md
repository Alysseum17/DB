...
* **Потенційна помилка:** Якби таблиця `questions` мала складений первинний ключ `(quiz_id, question_text)` і містила атрибут `quiz_difficulty` (складність вікторини), це призвело б до порушення 2NF. Атрибут `quiz_difficulty` залежав би лише від частини ключа (`quiz_id`), а не від конкретного питання.
* **Рішення в схемі:** Використано сурогатні первинні ключі (`serial id`) для всіх основних сутностей. Атрибути вікторини (складність, опис) винесені в батьківську таблицю `quizes` і не дублюються в дочірній таблиці `questions`.
* **Результат:** Усунено можливість часткових залежностей.
